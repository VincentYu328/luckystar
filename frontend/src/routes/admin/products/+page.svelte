...
<script>
  ...
  const loadError = data.loadError; // ⭐ 新增: 接收加载错误
</script>

<div class="space-y-8">
  <div class="flex justify-between items-center">
    <div>
      <h1 class="text-3d tracking-tight text-3xl">Products（产品列表）</h1>
      ...
      {#if loadError}
        <p
          class="text-sm text-red-700 mt-2 p-3 border border-red-300 bg-red-50 rounded-lg"
        >
          加载产品数据失败：{loadError}。请检查后端服务和权限配置。
        </p>
      {/if}
    </div><a
  ...
  <div class="bg-white border rounded-lg shadow-sm overflow-hidden">
    <table class="w-full text-left">
      <thead class="border-b bg-gray-50 text-sm uppercase text-gray-500">
        <tr>
          <th class="p-3">SKU（货号）</th> <th class="p-3">Name（名称）</th>
