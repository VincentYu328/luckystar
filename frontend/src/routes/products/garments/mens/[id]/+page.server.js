...
<div class="max-w-6xl mx-auto py-10 grid grid-cols-1 lg:grid-cols-2 gap-10">
  ...
  <div class="space-y-4">
    ...
    <!-- ⭐ 主图 -->
    <img
      src={getImageUrl(images.find(i => i.is_primary)?.url || images[0].url)}
      alt={product.name}
  ...
  <div class="space-y-5">
    ...
    <div class="text-gray-700 space-y-1">
      {#if product.color}<div><strong>Color:</strong> {product.color}</div>{/if}
      {#if product.style}<div><strong>Style:</strong> {product.style}</div>{/if}
      {#if product.size_label}<div><strong>Size:</strong> {product.size_label}</div>{/if}
      {#if product.gender}<div><strong>Gender:</strong> {product.gender}</div>{/if}
      {#if product.material}<div><strong>Material:</strong> {product.material}</div>{/if}
    </div>
    ...
    <div class="pt-2">
      <button class="px-6 py-3 bg-black text-white rounded-lg hover:bg-gray-800 w-full">
        Make an Inquiry
      </button>
    </div>
  </div>
